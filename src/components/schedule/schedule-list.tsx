  ...
  if (schedules.length === 0) {
    return (
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 text-center">
        ...
        <p className="text-gray-600 font-medium mb-1">No scheduled scans</p>
        <p className="text-sm text-gray-400 mb-4">Set up automated scans to monitor your CPQ config.</p>
        <button
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
      <div className="px-6 py-4 border-b border-gray-100">
        <h3 className="text-lg font-semibold text-gray-900">Scheduled Scans</h3>
      </div>
          ...
          return (
            <div key={schedule.id} className="px-6 py-4 flex items-center gap-4">
              ...
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${
                schedule.enabled ? 'bg-blue-50' : 'bg-gray-50'
              }`}>
                <Icon className={`w-5 h-5 ${schedule.enabled ? 'text-blue-600' : 'text-gray-400'}`} />
              </div>

              {/* Description */}
              <div className="flex-1 min-w-0">
                <p className={`text-sm font-medium ${schedule.enabled ? 'text-gray-900' : 'text-gray-400'}`}>
